...
def output_answer(new_list: list) -> Union[List[Dict[str, Union[str, Any]]], str]:
	...
	В функции реализован шаблон обработки полученного
	по запросу списка, в результирующий список
	...
	try:
		for i in range(len(new_list)):
			...
			dist_value = new_list[i].get("landmarks")[0].get('distance')[:-6]
			distance_value = round(float(dist_value) * 1.61, 2)
...
def output_message(answer: list, bot: telebot.TeleBot, user) -> None:
	"""
	...
	if user.is_city:
		...
		elif user.user_vars['method'] == 'highprice':
			key_1 = telebot.types.InlineKeyboardButton(
				text='Дешевые отели',
		...
		key_3 = telebot.types.InlineKeyboardButton(
			text='Поиск в другом городе',
			callback_data='answ|city'
		)
		keyboard.add(key_3)
		bot.send_message(
			user.chat_id,
			f'Выберите, что мы ищем:',
			reply_markup=keyboard
		)
	else:
		user.user_vars['id_city'] = None
		user.user_vars['num_hotels'] = None
		user.user_vars['method'] = None
