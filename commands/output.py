...
def output_message(message, answer: list, bot, User) -> None:
	...
	if User.users[message.chat.id].is_city:
		...
		elif User.users[message.chat.id].user_vars['method'] == 'highprice':
		...
		User.users[message.chat.id].user_vars['id_city'] = None
		User.users[message.chat.id].user_vars['num_hotels'] = None
		User.users[message.chat.id].user_vars['method'] = None
